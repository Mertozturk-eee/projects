...
void serialEvent (Serial myPort) { 
  ...
  data = myPort.readStringUntil('.'); // Veri sona kadar bekle
  data = data.substring(0, data.length()-1); // Son noktayı sil
  index1 = data.indexOf(","); 
  angle = data.substring(0, index1); 
  distance = data.substring(index1+1, data.length()); 

  iAngle = Integer.parseInt(angle); // Convert angle to integer
  iDistance = Integer.parseInt(distance); // Convert distance to integer
}
